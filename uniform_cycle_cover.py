...
def uniform_cycle_cover(H, two_cycles = False):
    ...
    while 0 != 1:
        ...
        nx.draw(D, pos = {x:x for x in g.nodes()}, node_color = [ncolors[x] for x in g.nodes()], width = 3)
...
dg = uniform_cycle_cover(nx.grid_graph([8, 8]), two_cycles = False)
...
nx.draw(dg[0], pos = {x:x for x in dg[0].nodes()}, node_color = [ncolors[x] for x in dg[0].nodes()], width = 3)
